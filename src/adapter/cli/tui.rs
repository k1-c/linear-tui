...
//! its control channel (`adapter::control`).
//!
...
use crate::adapter::control::{self, Command};
use crate::adapter::snapshot::{self, Shelf};
...
fn control_file(cwd: &std::path::Path) -> Result<PathBuf> {
    ...
    match crate::usecase::instance::to_report(&instances) {
