...
use crate::adapter::auth::token::{Account, TokenStore};
use crate::adapter::snapshot::Shelf;
...
pub fn run(args: &[String]) -> Result<()> {
    ...
    let workspace = crate::adapter::snapshot::workspace_of(&cwd);
    let instances = Shelf::new(&crate::adapter::snapshot::state_dir()?, &workspace).instances();
    ...
    let now = crate::adapter::snapshot::unix_now();
...
pub fn render(
    ...
    let ago =
        crate::adapter::snapshot::ago(&s.updated_at, now).unwrap_or_else(|| s.updated_at.clone());
...
mod tests {
    ...
    fn a_view_of_another_workspace_than_the_issue_commands_is_flagged() {
        ...
        let account = |id: &str| Account {
            organization: Some(crate::entity::Organization {
                id: id.into(),
                name: "Other".into(),
                url_key: "other".into(),
            }),
            user: None,
            tokens: crate::adapter::auth::token::OAuthTokens {
    ...
    fn the_markdown_says_what_is_open_and_marks_the_cursor() {
        ...
        let now = crate::adapter::snapshot::parse_timestamp(&snapshot.updated_at).unwrap() + 120;
    ...
    fn a_closed_instance_is_flagged_as_stale() {
        let snapshot = fixture();
        let now = crate::adapter::snapshot::parse_timestamp(&snapshot.updated_at).unwrap() + 7200;
