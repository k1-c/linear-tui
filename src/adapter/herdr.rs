...
use crate::entity::{AgentLink, Handoff};
...
pub async fn deliver(handoff: &Handoff) -> Result<()> {
    ...
    crate::adapter::private_file::write(&path, &serde_json::to_vec_pretty(&envelope)?)?;
...
mod tests {
    ...
    fn the_agents_file_parses_and_tolerates_new_states() {
        ...
        assert_eq!(agents[1].status, crate::entity::AgentStatus::Unknown);
        assert!(
