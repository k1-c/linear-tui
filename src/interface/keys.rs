...
use crate::interface::app::{App, FormField, Input, InputMode, Nav, Popup, Screen, TeamSection};
...
pub fn handle_key(app: &mut App, key: KeyEvent) {
    ...
    if app.view.popup == Popup::Palette {
        crate::interface::palette::handle_key(app, key);
        return;
    }
...
mod tests {
    ...
    use crate::interface::app::{IssueSource, Screen};
    ...
    fn enter_on_every_issue_list_opens_the_highlighted_issue() {
        ...
        for (screen, nav) in [
            ...
            app.store.issues[IssueSource::Project].items = reordered();
            app.store.issues[IssueSource::Cycle].items = reordered();
            app.set_preset(crate::interface::grouping::Preset::All);
            app.outbox.requests.clear();
            if let Some(nav) = nav {
                app.nav.dest = nav;
            }
