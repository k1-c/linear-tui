...
use crate::core::entity::{Issue, Priority};
use crate::core::usecase::Request;
use crate::interface::tui::app::{IssueSource, Popup, Screen};
...
use crate::core::entity::{User, WorkflowState};
...
fn a_picker_narrows_as_you_type_and_enter_takes_the_top_match() {
    ...
    assert!(matches!(
        app.outbox.requests.back(),
        Some(Request::Issue(crate::core::usecase::issue::Request::SetStatus { state_id, .. })) if state_id.as_str() == "s-prog"
    ));
}
...
use crate::interface::tui::app::{Nav, TeamSection};
...
fn searches(app: &App) -> Vec<(String, u64)> {
    app.outbox
        ...
        .filter_map(|r| match r {
            Request::Issue(crate::core::usecase::issue::Request::QuickSearch { term, seq }) => {
                Some((term.clone(), *seq))
            }
            _ => None,
        })
