...
use super::*;
use crate::core::entity::snapshot::{self as snap, ViewSnapshot};
...
mod tests {
    ...
    fn a_snapshot_round_trips_through_a_restore() {
        ...
        let saved = before
            .snapshot(&origin(), "2026-09-25T10:00:00Z".into())
            .unwrap();
        assert_eq!(
            saved.destination,
        ...
        assert!(requested(
            &after,
            |r| matches!(r, Request::Issue(crate::core::usecase::issue::Request::TeamIssues { team_id, .. }) if team_id == "t2")
        ));
    ...
    fn a_view_is_found_by_id_after_the_views_reorder() {
        ...
        assert!(!requested(&app, |r| matches!(
            r,
            Request::Issue(crate::core::usecase::issue::Request::TeamIssues { .. })
        )));
        assert!(
            app.snapshot(&origin(), "2026-09-25T10:00:00Z".into())
                .is_none(),
        ...
        assert!(requested(
            &app,
            |r| matches!(r, Request::Issue(crate::core::usecase::issue::Request::ViewIssues { view_id, .. }) if view_id == "v2")
        ));
    }
    ...
    fn a_deleted_team_falls_back_to_the_first_team() {
        ...
        assert!(requested(
            &app,
            |r| matches!(r, Request::Issue(crate::core::usecase::issue::Request::TeamIssues { team_id, .. }) if team_id == "t1")
        ));
    }
    ...
    fn an_issue_opened_by_identifier_takes_its_real_id_when_it_arrives() {
        ...
        assert!(requested(&app, |r| matches!(
            r,
            Request::Issue(crate::core::usecase::issue::Request::Detail { issue_id }) if issue_id == "ENG-9"
        )));
        assert!(requested(&app, |r| matches!(
            r,
            Request::Issue(crate::core::usecase::issue::Request::TeamIssues { .. })
