...
use crate::interface::app::{App, Chip, IssueSource, ListRow, Nav, Popup, Screen};
use crate::interface::grouping::Preset;
...
fn the_palette_draws_its_matches_where_it_records_them() {
    ...
    crate::interface::palette::click(&mut app, area.x + 2, area.y);
...
fn every_screen_renders_at_every_size() {
    ...
    let setups: [fn(&mut App); 16] = [
        ...
        |app| app.go_to_team_section(crate::interface::app::TeamSection::Projects),
        |app| app.go_to_team_section(crate::interface::app::TeamSection::Cycles),
...
fn every_documented_binding_appears_in_the_help_overlay() {
    ...
    for binding in crate::interface::keys::BINDINGS
        .iter()
...
mod timings {
    ...
    fn timings() {
        ...
        let mut cache = crate::interface::ui::Cache::default();

        let mut list = app();
        long_list(&mut list, 1000);
        time("list frame, 1000 issues", 500, || {
            terminal
                .draw(|f| crate::interface::ui::draw(f, &mut list, &mut cache))
                .unwrap();
        });
        ...
        time("detail frame, no comments", 500, || {
            terminal
                .draw(|f| crate::interface::ui::draw(f, &mut thread, &mut cache))
                .unwrap();
        });
