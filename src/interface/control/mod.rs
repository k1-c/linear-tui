...
//!
//! [`screen`] is what an agent reads; [`notation`] is how it writes keys.
...
pub async fn listen(path: PathBuf) -> Result<(mpsc::UnboundedReceiver<Asked>, EndpointFile)> {
    ...
    crate::private_file::write(&path, serde_json::to_string(&endpoint)?.as_bytes())?;
