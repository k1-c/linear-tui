//! The view snapshot: a record of what a linear-tui instance is showing —
//! the page, the issue open on it, and the list with the row under the
//! cursor. A value: it is replaced whole as the view changes, and two are
//! the same when they record the same view ([`ViewSnapshot::same_view`]).
//! The instance it belongs to is `super::Instance`.
//!
...
use super::ids::{CustomViewId, CycleId, FavoriteId, IssueId, OrganizationId, ProjectId, TeamId};
