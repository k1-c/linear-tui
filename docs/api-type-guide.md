...
When adding or modifying types in `src/core/entity/` (one file per aggregate —
...
- [ ] **Ids**: An entity id is its own newtype from `src/core/entity/ids.rs` (`IssueId`, `TeamId`, …), not a bare `String`; see [Ids](#ids)
...
3. **Define type**: Add struct/enum in the aggregate's file under `src/core/entity/` with appropriate serde attributes
4. **Add fixture**: Save the sample response to `tests/fixtures/<query_name>.json`
5. **Add test**: Write a deserialization test in `src/infra/linear/decode_tests.rs`, including a case where the new field is absent
6. **Add the query**: Use the `connection` or `mutate` helper in `src/infra/linear/client.rs` (see [Queries](#queries)) and cover it with a wiremock test there
...
`src/infra/linear/client.rs`, interpolated into each query with `format!`. Add new issue
fields there so list rows and detail responses stay interchangeable, rather than
extending a single query's selection set.

## Ids

Every entity id gets a newtype from `src/core/entity/ids.rs`, generated by the
