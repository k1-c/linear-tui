...
5. **Add test**: Write a deserialization test in the `tests` module of `src/api/types.rs`, including a case where the new field is absent
6. **Add the query**: Use the `connection` or `mutate` helper in `src/api/client.rs` (see [Queries](#queries)) and cover it with a wiremock test there
...
Every entity id gets a newtype from `src/api/ids.rs`, generated by the
